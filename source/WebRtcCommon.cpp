...
VOID onConnectionStateChange(UINT64 customData, RTC_PEER_CONNECTION_STATE newState)
{
    ...
    // changed order in C++ due to error: transfer of control bypasses initialization of
...
STATUS createSampleConfiguration(PCHAR channelName, IotCoreCredential *pIotCoreCredential, SIGNALING_CHANNEL_ROLE_TYPE roleType, BOOL trickleIce, BOOL useTurn, UINT32 logLevel,
                                 PSampleConfiguration *ppSampleConfiguration)
{
    ...
    CHK_ERR((pIotCoreCredentialEndPoint = pIotCoreCredential->pIotCoreCredentialEndPoint) != NULL, STATUS_INVALID_OPERATION,
            "AWS_IOT_CORE_CREDENTIAL_ENDPOINT must be set");
    CHK_ERR((pIotCoreCert = pIotCoreCredential->pIotCoreCert) != NULL, STATUS_INVALID_OPERATION, "AWS_IOT_CORE_CERT must be set");
    CHK_ERR((pIotCorePrivateKey = pIotCoreCredential->pIotCorePrivateKey) != NULL, STATUS_INVALID_OPERATION, "AWS_IOT_CORE_PRIVATE_KEY must be set");
    CHK_ERR((pIotCoreRoleAlias = pIotCoreCredential->pIotCoreRoleAlias) != NULL, STATUS_INVALID_OPERATION, "AWS_IOT_CORE_ROLE_ALIAS must be set");
#else
    ...
    if ((pSampleConfiguration->channelInfo.pRegion = pIotCoreCredential->pKVSRegion) == NULL)
    {
    ...
    pSampleConfiguration->pCaCertPath = pIotCoreCredential->pIotCoreCaCertPath;
